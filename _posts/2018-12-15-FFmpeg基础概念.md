本文首发地址：<https://www.cnblogs.com/leisure_chn/p/10297002.html>，转载请注明出处  
...
### 4.2 滤镜链的使用  
**滤镜链(filterchain)**是以逗号分隔的filter序列，语法如下：  
...
### 4.3 滤镜图的使用  
**滤镜图(filtergraph)**通常是以分号分隔的filterchain序列。filtergraph分为简单filtergraph和复杂filtergraph。  
filtergraph的语法如下：  
`filter1;fiter2;filter3;...;filterN-2;filterN-1;filterN`  
...
#### 4.3.2 复杂滤镜图  
复杂滤镜图(filtergraph)用于简单filtergraph处理不了的场合。比如，多路输入流和(或)多路输出流，或者输出流与输入流类型不同。  
...
## 7. 选项  
如下内容引用自“[FFmpeg参数中文详细解释](https://blog.csdn.net/leixiaohua1020/article/details/12751349)”  
...
## 8. 示例  
如下内容主要引用自“[FFmpeg常用基本命令](http://www.cnblogs.com/dwdxdy/p/3240167.html)”  
