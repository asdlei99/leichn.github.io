...
音视频同步是播放器中比较复杂的一部分内容。前几次实验中的代码远不能满足要求，需要大幅修改和增加代码。本次实验将不在前几次的代码基础上修改，而是基于ffplay源码进行修改。ffplay是FFmpeg工程自带的一个简单播放器，尽管称为简单播放器，其代码实现仍显得过为复杂。本实验对ffplay.c进行删减，删掉复杂的命令选项、滤镜操作、SEEK操作、逐帧插放等功能，仅保留最核心的音视频同步部分。  

播放器的基本原理和大致流程不会改变，前面几次实验仍具有有效参考意义。  
