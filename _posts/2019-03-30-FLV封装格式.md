...
| 字段      | 长度 | 说明 |
|:----------|:-----|:-----|
| VideoData | IF FrameType == 5<br>　UI8<br>ELSE (<br>IF CodecID == 2<br>　H263VIDEOPACKET<br>IF CodecID == 3<br>　SCREENVIDEOPACKET<br>IF CodecID == 4<br>　VP6FLVVIDEOPACKET<br>IF CodecID == 5<br>　VP6FLVALPHAVIDEOPACKET<br>IF CodecID == 6<br>　SCREENV2VIDEOPACKET<br>IF CodecID == 7<br>　AVCVIDEOPACKET<br>) | 视频帧净荷数据或视频帧信息<br>除 AVCVIDEOPACKET 外的所有格式都可以参考 SWF 文件格式规范|
```c
typedef struct {
  IF FrameType == 5
    UI8 VideoData;
...
[2] [FLV 存在 B 帧情况下的 DTS 和 PTS](https://www.jianshu.com/p/1cbe31baa711), <https://www.jianshu.com/p/1cbe31baa711>  
